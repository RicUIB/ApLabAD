...
reposición. Sea $X$ la edad media de los dos estudiantes seleccionados.
Hallar la función de probabilidad para $X$.
...
sol_df=data.frame(Media_Edad=x,Freq_Absolutas=as.numeric(CF),
                  Probabilidades=as.numeric(probs))
knitr::kable(sol_df)
```
...
-   Es solo continua por la derecha, luego es discreta no es continua
    con dominio $D_X=\{3,4,5,6\}$ que son los valores dónde
    $P(X=x)=F_X(x)-F_X(x^{-})\not=0$.
...
La variable aleatoria $Z$ tiene por función de probabilidad: $$f_Z (x)=
\left\{\begin{array}{ll}
...
Tenemos que $\sum_{x=1}^n k\cdot x=1$ y tenemos que determinar $k$ en
función de $n$, tenemos que
...
$X$. Calcular también $P\left(-{1\over 2}\leq X\leq {1\over 2}\right)$.
...
3.  Calcular las siguientes probabilidades: $P(X\geq 0)$ y
...
Como $D_X=[-1,1]$ entonces $Y=15+2 X$ varía desde $Y=15+2\cdot (-1)=13$
hasta $Y=15+2\cdot 1=17$ y por lo tanto su dominio es $D_Y=[13,17].$
...
$f_X(x)$. Consideramos la variable aleatoria $Y=\mathrm{e}^X$. Hallar la
función de densidad de la variable aleatoria $Y$, $f_Y(y)$.
