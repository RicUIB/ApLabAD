---
title: "SOLUCIONES: Ejercicios Tema 3 Parte 2 - Distribuciones Notables: continuas"
...
Tenemos que $X$ es $N(\mu=3.45,\sigma=10)$   tenemos que $P(X <4)=`r round(pnorm(4,mean=3.45,sd=1),4)`$

En segundo lugar nos piden $PX(>3.5)=1-P(X\leq 3.5)=`r 1- round(pnorm(3.5,mean=3,sd=10),4)`$


Los cálculos los podemos hacer con R

```{r}
round(pnorm(4,mean=3.45,sd=1),4)# apartado 1. P(X<4)
round(1-pnorm(3.5,mean=3.45,sd=1),4)# apartado 2. P(X>3.5)
```
...
1. En 60 minutos recibe 100 llamadas así que en un minuto recibe $\lambda=\frac{102}{60}=`r 102/60`$. Luego $X=$ tiempo entre dos llamadas consecutivas en minutos  sigue una ley $Exp(\lambda=1.7)$

2. $P(X>2.5)=1-P(X\leq 2.5)=`r round(1-pexp(2.5,rate=1.7),4)`.$


3. $P(X<3)=P(X\leq 3)=`r round(pexp(3,rate=1.7),4)`.$

4. $E(X)=\frac{1}{\lambda}=\frac{1}{1.7}=`r round(1/1.7,4)`$ y   $Var(X)=\frac{1}{\lambda^2}=\frac{1}{1.7^2}=`r round(1/1.7^2,4)`.$ 
...
knitr::include_graphics("pro3_cont_1.png")
```
...
1. $P(560000\leq X\leq 660000)=P(X\leq   660000)-P(X\leq 560000)=`r round(pnorm(660000,mean=600000,sd=60000),4)`-`round(pnorm(560000,mean=600000,sd=60000),4)`.$
...
2. Nos piden  el valor $x_0$ tal que $P(X>x_0)=0.2$, es decir el valor que supera el 20\% de las  viviendas más caras.  Este valor será el que deje por debajo  el coste del 89\% de las casas por lo que es el cuantil 0.8 lo calculamos con R (ejercicio utiliza  google sheets para obtener el mismo resultado)
...
3. Ahora somos más ambiciosos y que remos gastar para estar entre el 5\% de casas más caras. De manera similar al caso anterior queremos calcular el cuantil $x_{0.95}$, lo haremos con R
...
knitr::include_graphics("pro7_cont_1.png")
```
