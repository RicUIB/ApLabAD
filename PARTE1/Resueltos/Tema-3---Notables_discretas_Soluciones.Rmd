...
\begin{array}{lll}
P(1\leq X< 4)&=& P(X<4)-P(X<1)=P(X\leq 3)-P(X=0)\\&=&
`r pbinom(3,size=5,prob=1/6)-dbinom(0,size=5,prob=1/6)`.
\end{array}
$$
...
$P(X=0)=`r dbinom(0,size=20,prob=0.05)`.$
...
$$
\begin{array}{lll}
P\left(X_{\frac13}\geq 2 \right)&=&1-P\left(X_{\frac13}< 2 \right)=1-P\left(X_{\frac13}\leq 1\right)\\
&=&
1-\displaystyle\sum_{x=0}^1 \frac{1^x}{x!} e^{-1}
\\
&=&
1-\left(\frac{1^0}{0!} e^{-1}+\frac{1^1}{1!} e^{-1}\right)\\
&=&
1-e^{-1}\left(1+1\right)=1-2\cdot e^{-1}=`r  1-2*exp(-1)`.
\end{array}
$$

Con R, estas dos instrucciones dan la solución
...
**Apartado 3.** La variable $T$ tiempo entre dos ventas consecutivas  de  este proceso poisson es una v.a. $Exp(\lambda=1)$. Ahora  nos piden 3 periodos de 2 semanas en total 6 semanas $P(T>6)=1-P(X\geq 6)=1-\left(1-e^{-1\cdot 6}\right)=`r 1-(1-exp(-6))`.$
...
En este caso no hay manera de asociarlo a una distribución notable. o al menos no la hemos visto. LO resolveremos de forma directa escribiendo todos los casos. Sea
