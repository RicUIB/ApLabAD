...
Se supone que el lector conoce la teoría básica dde conjuntos, operaciones entrde conjuntos, aplicaciones entrde conjuntos y principios básicos de numerabilidad.
...
::: callout-body
Sea $\Omega$ un conjunto y $\mathcal{A}\subseteq \mathcal{P}(\Omega)$ una familia de subconjuntos de $\Omega$. Diremos que $\mathcal{A}$ es una **álgebra** sobre $\Omega$ si:

1. $\Omega\in\mathcal{A}$.
2. Si $A\in\mathcal{A}$, entoncess $A^c\in\mathcal{A}$.
3. Si $A_1,\ldots,A_n\in\mathcal{A}$, entoncess $\bigcup_{i=1}^{n} A_i\in\mathcal{A}$, para todo $n\in\mathbb{N}$.

Si, además, $\mathcal{A}$ es cerrada respecto a **uniones numerables**, es decir, si para toda familia $(A_i)_{i=1}^{\infty}\subseteq\mathcal{A}$ se cumple que $\bigcup_{i=1}^{\infty} A_i\in\mathcal{A}$, entoncess diremos que $\mathcal{A}$ es una **$\sigma$-álgebra** sobre $\Omega$.

El par $(\Omega,\mathcal{A})$ se llama **espacio medible** y los elementos de $\mathcal{A}$ se denominan **conjuntos medibles** (o **sucesos**).
:::
:::::
...
2. $\mathcal{A}$ es cerrada respecto a **intersecciones finitas y numerables** y respecto a la **diferencia dde conjuntos**. En particular, si $A,B\in\mathcal{A}$, entoncess $A\cap B\in\mathcal{A}$ y $A\setminus B\in\mathcal{A}$.
3. (Continuidad para uniones crecientes) Si $(A_i)_{i=1}^{\infty}\subseteq\mathcal{A}$ cumple $A_i\subseteq A_{i+1}$ para todo $i$, entoncess $\bigcup_{i=1}^{\infty} A_i\in\mathcal{A}$.
4. (Continuidad para intersecciones decrecientes) Si $(A_i)_{i=1}^{\infty}\subseteq\mathcal{A}$ cumple $A_{i+1}\subseteq A_i$ para todo $i$, entoncess $\bigcap_{i=1}^{\infty} A_i\in\mathcal{A}$.
:::
:::::
...
::: callout-body
Sean $\mathcal{A}$ y $\mathcal{B}$ dos $\sigma$-álgebras de $\Omega$ entoncess
...
La $\sigma$- álgebra generada por estos productos dde conjuntos medibles se llama $\sigma$-**álgebra producto** 
...
2. Si $\{A_n\}_{n\geq 1}$ es una sucesión creciente dde conjuntos de $\mathcal{D}$, entoncess $\cup_{n=1}^{\infty} A_n \in \mathcal{D}.$
3. Si $A,B\in \mathcal{D}$ y $A\subset B$, entoncess $B - A \in \mathcal{D}$.
:::
::::: 
...
1. **Aditividad numerable ($\sigma$-aditividad):** 
  Si $(A_n)$es una sucesión dde conjuntos **disjuntos** en $\mathcal{A}$, entoncess:
...
2. **Medida $\sigma$-finita:** 
  Si $\Omega$ puede cubrirse con una colección **numerable** dde conjuntos medibles de **medida finita**. 
...
### **Ejemplos**
1. **Medida de conteo:** 
  Si $\mu(A)$ es simplemente el número de elementos de$A$ (cuando$A$ es finito o numerable), entoncess es una medida finita si $\Omega$ es finito, pero puede ser infinita si $\Omega$es infinito.
...
**Demostración**: 
Sea $A = \bigcup_n A_n$. Si $\mu(A_n) = +\infty$ para algún $n$, entoncess $\mu(A_k) = \infty$ para todo $k \geq n$ y el resultado es trivial. 
...
- La **continuidad hacia arriba y hacia abajo** garantiza que la medida respeta límites de secuencias dde conjuntos.
